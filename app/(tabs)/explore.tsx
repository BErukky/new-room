...
import { View, Text, ScrollView, TouchableOpacity, TextInput, ActivityIndicator, Share, RefreshControl } from 'react-native';
...
const CATEGORIES = ['All', 'Crypto', 'AI', 'Electronics', 'Startups', 'Finance', 'Sports', 'Gaming'];
  ...
  const fetchExploreNews = useCallback(async () => {
    ...
    try {
      ...
      if (searchQuery.trim()) {
        ...
        news = await newsService.searchNews('artificial intelligence', language);
  ...
  return (
    <SafeAreaView style={{ flex: 1 }} className="bg-slate-50 dark:bg-slate-900" edges={['top']}>
      ...
      <View className="px-6 pt-4 pb-2 flex-row items-center justify-between">
        <Text className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Explore</Text>
        ...
        >
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                <Text className={`font-semibold ${isActive ? 'text-white' : 'text-slate-600 dark:text-slate-300'}`}>
                  {cat}
                </Text>
              </TouchableOpacity>
            );
      ...
      >
        {isLoading ? (
          <View className="py-20 justify-center items-center">
            ...
            <Text className="mt-4 text-slate-500 font-medium">Fetching latest news...</Text>
          </View>
        ) : (
            ...
            <View className="space-y-6">
                ...
                >
                  ...
                  <View className="flex-row items-center mb-3">
                    <View className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/40 items-center justify-center">
                      <Text className="text-blue-600 dark:text-blue-400 font-bold text-xs">{news.author?.charAt(0) || 'N'}</Text>
                    </View>
                    <View className="ml-3 flex-1">
                      <Text className="font-bold text-slate-900 dark:text-white" numberOfLines={1}>{news.author || 'Global News'}</Text>
                      <Text className="text-xs text-slate-500">{timeAgo(news.published)} • {news.category[0] || 'General'}</Text>
                    </View>
                    <TouchableOpacity>
                  ...
                  <View className="flex-row items-center justify-between mt-4 px-2">
                    <View className="flex-row items-center gap-6">
                      ...
                      >
                        ...
                        />
                        <Text className="text-slate-500 ml-2">{likesCounts[news.id] || 0}</Text>
                      </TouchableOpacity>
                      <TouchableOpacity 
                    ...
                    <TouchableOpacity onPress={() => Share.share({ message: news.title + ' ' + news.url })}>
              ...
              {feed.length === 0 && (
                <View className="py-10 items-center">
                  ...
                  <Text className="text-slate-400 mt-4 text-center">No articles found for this category.</Text>
                </View>
              )}
            </View>
          </View>
