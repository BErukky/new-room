...
export default function ArticleScreen() {
  const { id, title, image, author, published, description, url } =
  ...
  const toggleBookmark = async () => {
    if (!user)
      return Toast.show({
        type: "info",
        text1: "Login Required",
        text2: "Please log in to save articles.",
      });
    ...
    try {
      ...
      Toast.show({
        type: "success",
        text1: isBookmarked ? "Removed from Saved" : "Saved to Library",
      });
    } catch (e: any) {
  ...
  const handleSendComment = async () => {
    ...
    try {
      if (editingCommentId) {
        ...
        setCommentsList(
          commentsList.map((c) => (c.id === editingCommentId ? { ...c, content: commentText.trim() } : c))
        );
    ...
    <View style={{ flex: 1 }} className="bg-white dark:bg-slate-900">
      ...
      >
        ...
        <View className="relative w-full h-80">
          ...
          <SafeAreaView
            edges={["top"]}
            className="absolute w-full px-4 pt-2 flex-row justify-between items-center"
          >
            <TouchableOpacity
              onPress={() => router.back()}
            ...
            <View className="flex-row gap-3">
              ...
              >
                <Ionicons
                  ...
                  size={22}
                  color={isBookmarked ? "#3b82f6" : "white"}
                />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() =>
        ...
        <View className="px-6 py-6 bg-white dark:bg-slate-900 -mt-6 rounded-t-3xl">
          ...
          <View className="border-t border-slate-100 dark:border-slate-800 pt-6 mt-4 pb-24">
            <Text className="text-xl font-bold text-slate-900 dark:text-white mb-6">
              {t("comments")} ({commentsList.length})
            </Text>
            ...
            <View className="space-y-6">
              ...
              {commentsList.length === 0 && (
                <Text className="text-slate-400 italic text-center py-4">
      ...
      >
        <SafeAreaView
          edges={["bottom"]}
          className="bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 px-6 py-3 flex-row items-center justify-between"
        >
          <View className="flex-row items-center bg-slate-100 dark:bg-slate-800 rounded-full px-4 py-2 flex-1 mr-4 border border-slate-200 dark:border-slate-700">
            <TextInput
              ...
              className="flex-1 ml-1 py-2"
              placeholder={editingCommentId ? "Edit comment..." : t("add_comment")}
            ...
            {commentText.length > 0 && (
              <TouchableOpacity onPress={handleSendComment}>
                <Ionicons name={editingCommentId ? "checkmark" : "send"} size={20} color="#3b82f6" />
              </TouchableOpacity>
            )}
          </View>
